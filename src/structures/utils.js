...
const { Routers, Locale } = require('discord.js');
...
module.exports = class util {
    ...
    async *loadFiles(dir) {
        ...
        const targetFiles = files.filter(file => { return path.extname(file).toLowerCase() !== EXTENSION; });
    ...
    async LoadInteractions() {
        ...
        for await (const interactionFile of this.loadFiles(`${this.directory}interactions`)) {
            ...
            let interaction = new File(this.client, name);
        ...
        try {
            ...
            await res.put(Routers.applicationCommands(this.client.user.id), { body: int });
        } catch (err) {
            throw new Error(err);
        }
    }
